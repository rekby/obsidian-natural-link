...
npm run lint         # ESLint
npm run dict:ru:suffix # Rebuild Russian suffix rules from OpenCorpora
npm run dict:ru:build # Rebuild Russian irregular forms from OpenCorpora
npm run dict:en:build # Rebuild English irregular forms from WordNet 3.1 exception lists
...
- Irregular forms support is dictionary-based.
  - Russian map is generated from OpenCorpora (`dict.opcorpora.xml.bz2`) via build-time scripts in `scripts/dictionaries/`.
  - English map is generated from WordNet 3.1 exception lists (`wn3.1.dict.tar.gz`, `*.exc`) via build-time scripts in `scripts/dictionaries/`.
