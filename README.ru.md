...
- **Ссылки на заголовки и блоки**: Используйте `#` для ссылки на заголовок (`заметка#заголовок`) или `^` для ссылки на блок (`заметка^текст`). Плагин сначала ищет заметку, затем показывает подходящие заголовки или текстовые блоки с превью. По `^` отображаются все блоки текста заметки — блоки с существующими метками показывают их, а для блоков без меток уникальная метка генерируется автоматически. Используйте `|` для явного отображаемого текста (`заметка|текст`).
