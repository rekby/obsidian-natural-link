...
- **Морфологический поиск**: Находите заметки по любой форме слова. Поиск по «деревянную коробку» найдёт заметку «Деревянная коробка». Полностью поддерживается русская морфология, включая чередования согласных (`друг`/`дружить`, `ходить`/`хожу`) и супплетивные формы (`люди`/`человек`). Для английского учитываются Snowball-стемминг и словарь irregular-форм, например `children`/`child`, `went`/`go`, `better`/`good`.
...
- **Мультиязычность**: Русский и английский поиск работают одновременно. Для русского irregular-морфология основана на данных словаря [OpenCorpora](https://opencorpora.org/), для английского — на списках исключений WordNet 3.1.
...
npm run dict:ru:suffix # Извлечь русские суффиксные правила из OpenCorpora
npm run dict:ru:build  # Пересобрать русский словарь супплетивных форм из OpenCorpora
npm run dict:en:build  # Пересобрать словарь английских irregular-форм из WordNet 3.1
npm run dict:build     # Запустить все пайплайны сборки словарей (правила + словари)
```

## Источники словарей

Файлы данных для русского языка генерируются из морфологического словаря [OpenCorpora](https://opencorpora.org/):

- Формат источника: XML-словарь (`dict.opcorpora.xml.bz2`)
- Описание формата: [Формат экспорта OpenCorpora](https://opencorpora.org/?page=export)
- Страница скачивания: [Материалы для скачивания OpenCorpora](https://opencorpora.org/?page=downloads)
- Лицензия источника: [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/)

Данные словаря английских irregular-форм генерируются из списков исключений WordNet 3.1:

- Архив источника: `wn3.1.dict.tar.gz`
- Файлы исключений: `noun.exc`, `verb.exc`, `adj.exc`, `adv.exc`
- URL скачивания: <https://wordnetcode.princeton.edu/wn3.1.dict.tar.gz>

| Сгенерированный файл | Содержимое | Команда регенерации |
|---|---|---|
| `src/stemming/russian-suffix-rules.ts` | ~1 600 суффиксных правил (~64 КБ) | `npm run dict:ru:suffix` |
| `src/stemming/russian-irregular-forms.ts` | ~5 700 супплетивных пар форма→каноническая (~180 КБ) | `npm run dict:ru:build` |
| `src/stemming/english-irregular-forms.ts` | Пары английская irregular-форма→каноническая из исключений WordNet | `npm run dict:en:build` |

Запустите `npm run dict:build`, чтобы перегенерировать все словарные файлы. Сырые скачанные архивы хранятся в `.cache/dictionaries/` и в git не коммитятся.
