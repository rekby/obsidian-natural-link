...
- **Морфологический поиск**: Находите заметки по любой форме слова. Поиск по «деревянную коробку» найдёт заметку «Деревянная коробка». Полностью поддерживается русская морфология, включая чередования согласных (`друг`/`дружить`, `ходить`/`хожу`) и супплетивные формы (`люди`/`человек`).
...
- **Мультиязычность**: Русский и английский поиск работают одновременно. Морфологический анализ русского языка основан на данных словаря [OpenCorpora](https://opencorpora.org/).
...
npm run dict:build     # Запустить все пайплайны сборки словарей (правила + словарь)
```

## Источники словарей

Оба файла данных для русского языка генерируются из морфологического словаря [OpenCorpora](https://opencorpora.org/):
...
Запустите `npm run dict:build`, чтобы перегенерировать оба файла. Сырые скачанные архивы хранятся в `.cache/dictionaries/` и в git не коммитятся.
