...
*Экспериментальная функция: подменяет стандартные подсказки во время ввода на подсказки плагина:*
...
- **Встроенные подсказки `[[`** (опционально): Замените стандартное автодополнение ссылок Obsidian морфологическим поиском плагина. Подсказки появляются прямо в редакторе при вводе `[[`, без открытия отдельного модального окна. Включите в Настройки → Natural link → «Заменить стандартные подсказки [[».
