...
- **Tab как автодополнение в IDE**: Нажмите **Tab**, чтобы принять выбранную подсказку и вставить ссылку без отображаемого текста (`[[Заметка]]`, `[[Заметка#Заголовок]]`, `[[Заметка#^blockId]]`).
...
3. Выберите подходящую заметку:
   - **Enter** вставляет `[[Найденная заметка|ваш текст]]`.
   - **Tab** вставляет `[[Найденная заметка]]` (также работает для `#`-заголовков и `^`-блоков).
