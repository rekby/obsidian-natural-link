 ...
 */
export function normalizeYo(word: string): string {
...
export function normalizeConsonantAlternations(stem: string): string {
...
export function russianSnowballStem(word: string): string[] {
	const snowballStem = snowball.stem(normalizeYo(word));
	return [normalizeConsonantAlternations(snowballStem)];
}

/** @deprecated Use russianSnowballStem instead. Kept for backward compatibility. */
export const russianBaseStem = russianSnowballStem;
